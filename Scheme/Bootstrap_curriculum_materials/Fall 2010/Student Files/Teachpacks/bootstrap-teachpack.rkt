...
(provide START EXAMPLE test-frame score sq sine cosine tangent
...
(define (animate/proc title title-color
  ...
  (begin
    ...
    (let* ((player (make-being (make-posn (/ WIDTH 2) (/ HEIGHT 2)) playerImg))
           ...
           (targets (map (lambda (t) (reset (make-being (make-posn 0 0) t))) targetImgs))
           (objects (map (lambda (o) (reset (make-being (make-posn 0 0) o))) objectImgs))
           (projectiles empty)
